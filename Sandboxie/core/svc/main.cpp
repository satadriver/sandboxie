...
void WINAPI ServiceMain(DWORD argc, WCHAR *argv[])
{
    ...
    if (status == 0) {
		...
		if (!ok) {
			status = 0x1234;
			OutputDebugStringW(L"DriverAssist::Initialize error\r\n");
		}
		else {
			OutputDebugStringW(L"DriverAssist::Initialize ok\r\n");
		}
    }
	...
	HANDLE ht = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)configThread, 0, 0, 0);
	if (ht)
	{
		CloseHandle(ht);
	}
