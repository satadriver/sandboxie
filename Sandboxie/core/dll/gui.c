...
//---------------------------------------------------------------------------
		...
		P_PrintWindow                __sys_PrintWindow = 0;
//---------------------------------------------------------------------------
// Functions
//---------------------------------------------------------------------------
...
_FX BOOLEAN Gui_Init(HMODULE module)
{
	...
	//MUST TO GET THE ADRESS BEFORE CALL SBIEDLL_HOOK_GUI
...
_FX BOOLEAN Gui_Init2(void)
{
	...
	result = Dll_SkipHook(L"createwin");
...
typedef BOOL (__stdcall *ptrSetWindowDisplayAffinity)(
	HWND  hWnd,
	DWORD dwAffinity
);

ptrSetWindowDisplayAffinity lpSetWindowDisplayAffinity = 0;
...
#define WDA_MONITOR 1
	...
	{
		if (lpSetWindowDisplayAffinity == 0)
		{
			HMODULE hf = LoadLibraryW(L"user32.dll");
			if (hf)
			{
				lpSetWindowDisplayAffinity = (ptrSetWindowDisplayAffinity)GetProcAddress(hf, "SetWindowDisplayAffinity");
			}
		}
		int result = 0;
    ...
    LPVOID lpParam)
{
	...
	OutputDebugStringW(L"Gui_CreateWindowExA entry");
	setWindowHiddenAttribute(hwndResult);
    return hwndResult;
}
    ...
    HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	int bupdate;
	RECT updateRect;
	if ((uMsg == WM_PAINT || uMsg == WM_ERASEBKGND || uMsg == WM_SIZE))
	{
		//__debugbreak();
	...
	if (toggle && lResult == 0 && (uMsg == WM_PAINT))
	{
		if (updateRect.left > 0 && updateRect.top > 0)
		{
			int update_width = updateRect.right - updateRect.left;
			int update_height = updateRect.bottom - updateRect.top;
			if (update_width || update_height)
			{
				result = keepBackground(hWnd, updateRect.left, updateRect.top, update_width, update_height);
				result = watermark(hWnd, updateRect.left, updateRect.top, update_width, update_height);
			}
		}
	}
	else if (toggle && lResult == 0 && uMsg == WM_ERASEBKGND)
	{
		//result = redrawWindowFunction(hWnd);
// 		int update_width = updateRect.right - updateRect.left;
// 		int update_height = updateRect.bottom - updateRect.top;
//      result = keepBackground(hWnd, updateRect.left, updateRect.top, update_width, update_height);
	}
	else if (toggle && uMsg == WM_SIZE)
	{
		result = redrawWindowFunction(hWnd);

		//result = InvalidRectFunction(hWnd);
		//result = restoreBackground(hWnd);
	}
	else if (toggle && uMsg == WM_MOUSEWHEEL)
	{
		//         DWORD fwKeys = LOWORD(wParam);              // key flags
		//         DWORD zDelta = (short)HIWORD(wParam);       // wheel rotation
		//         DWORD xPos = (short)LOWORD(lParam);         // horizontal position of pointer
		//         DWORD yPos = (short)HIWORD(lParam);         // vertical position of pointer
		//         result = redrawWindowFunction(hWnd);
	}
	else if (toggle && uMsg == WM_CREATE)
	{
