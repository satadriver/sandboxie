...
P_NtOpenFile                 __sys_NtOpenFile                = NULL;
       P_NtCreateFile               __sys_NtCreateFile              = NULL;
 P_NtQueryAttributesFile      __sys_NtQueryAttributesFile     = NULL;
 P_NtQueryFullAttributesFile  __sys_NtQueryFullAttributesFile = NULL;
 P_NtQueryInformationFile     __sys_NtQueryInformationFile    = NULL;
...
static P_NtSetInformationFile       __sys_NtSetInformationFile      = NULL;
 P_NtDeleteFile               __sys_NtDeleteFile              = NULL;
...
static P_NtCreateMailslotFile       __sys_NtCreateMailslotFile      = NULL;
 P_NtReadFile                 __sys_NtReadFile                = NULL;
 P_NtWriteFile                __sys_NtWriteFile               = NULL;
...
// 	if ( wcsstr(TruePath, L"__debugbreak.txt") || wcsstr(CopyPath, L"__debugbreak.txt") )
// 	{
...
// 	}
    ...
    if (NT_SUCCESS(status)) {
		...
		if (/*HaveTrueFile &&*/ HaveCopyFile && (DesiredAccess & DELETE) && FileType &&
			(/*Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER ||*/ Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER_PLUSPLUS) &&
			(/*(CreateOptions & FILE_OPEN_REPARSE_POINT) ||*/ (CreateOptions & FILE_DELETE_ON_CLOSE)))
		{
			//__debugbreak();
		...
		//first time CreateOptions is FILE_OPEN_REPARSE_POINT,second time CreateOptions is FILE_DELETE_ON_CLOSE
		//first time ObjectName is \??\,second time ObjectName is \Device\HarddiskVolume1
		//if open in explorer++.exe(EXPLORER_PROCESS_NAME_PLUSPLUS),Dll_ImageType is DLL_IMAGE_SHELL_EXPLORER_PLUSPLUS
		if (FileType && (DesiredAccess & DELETE) &&
			(/*Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER ||*/ Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER_PLUSPLUS) &&
				(CreateOptions & FILE_DELETE_ON_CLOSE)  )
		{
			//__debugbreak();
			...
			if (myresult == FALSE)
			{
				if (HaveTrueFile == 'Y') {
					...
					if (FileType & FILE_NON_DIRECTORY_FILE)
					{
						...
						WCHAR fn[MAX_PATH];
						getFilename(CopyPath, fn);
						wcscat(recyclepath, fn);
					...
					{
						WCHAR recyclepath[MAX_PATH];
						myresult = createNewRecyclePath(CopyPath, recyclepath);
						myresult = copypath(TruePath, recyclepath, FALSE);
