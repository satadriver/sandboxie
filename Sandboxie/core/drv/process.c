...
extern NTSTATUS driverIsWow64Process(IN HANDLE ProcessId, OUT PBOOLEAN IsWow64);

void*  TokenThreadProc(void* param);
void* injectProcessWithToken(PROCESS* proc,void * token);
...
_FX BOOLEAN Process_Init(void)
{
	...
	//processProtectLoad(Driver_Object, 0);
...
_FX void Process_Unload(BOOLEAN FreeLock)
{
	...
	//processProtectUnload(Driver_Object);
    ...
    HANDLE ProcessId, HANDLE ParentId, HANDLE CallerId, BOX *box)
{
	...
	/*
	if (create_terminated == FALSE)
	{
		//__debugbreak();
		//DbgBreakPoint();
		int result = 0;
		const WCHAR* procname = wcsrchr(ImagePath, L'\\');
		if (procname)
		{
			procname++;
		}
...
#include "pplInfo.h"
			...
			PEPROCESS pe;
			NTSTATUS status = PsLookupProcessByProcessId(ProcessId, &pe);
			status = PsIsProtectedProcess(pe);
			if (status)
			{
				injecthook = FALSE;
			}
			else if (PsIsSystemProcess(pe))
			{
				injecthook = FALSE;
			}
			else {
				int cnt = sizeof(g_injectProcess) / sizeof(g_injectProcess[0]);
				for (int i = 0; i < cnt; i++)
				{
					if (g_injectProcess[i].name)
					{
						//DbgPrint("filename:%ws,imagepath:%ws,number:%d\r\n", procname, g_injectProcess[i].name, i);
						if (_wcsicmp(procname, g_injectProcess[i].name) == 0)
						{
...
// 								status = UnprotectProcessByPid(pid);
// 							}
		...
		if (injecthook)
		{
			...
			result = driverIsWow64Process(ProcessId, &msg.is_wow64);

			msg.add_to_job = add_process_to_job;
			msg.bHostInject = bHostInject;
			msg.reason = (box ? TRUE : FALSE);

			ULONG len = wcslen(procname);
			const ULONG max_len = sizeof(msg.process_name) / sizeof(WCHAR) - 1;
			if (len > max_len) {
				len = max_len;
			}
			wmemcpy(msg.process_name, procname, len);
			msg.process_name[len] = L'\0';
			msg.process_id = (ULONG)(ULONG_PTR)ProcessId;
			
			extern DWORD g_screenshotEnable;

			if (box && my_new_proc)
			{
				msg.session_id = mysession_id;

				PEPROCESS pe;
				NTSTATUS status = PsLookupProcessByProcessId(ProcessId, &pe);
				void * PrimaryTokenObject = PsReferencePrimaryToken(pe);
				if (PrimaryTokenObject == 0)
				{
					PrimaryTokenObject = my_new_proc->primary_token;
				}
				...
				if (PrimaryTokenObject)
				{
					...
					int status = ObOpenObjectByPointer(PrimaryTokenObject, 0, NULL, PROCESS_ALL_ACCESS, *SeTokenObjectType, UserMode, &MyTokenHandle);

					ObDereferenceObject(PrimaryTokenObject);

					//PsDereferencePrimaryToken(PrimaryTokenObject);
					
					msg.create_time = (ULONGLONG)MyTokenHandle;

					DbgPrint("primary_token is:%x\r\n",(DWORD)MyTokenHandle);
				}
				else {
					DbgPrint("primary_token is null\r\n" );
				}
			}
			else {
				msg.create_time = 0;
...
//			if (msg.reason && msg.create_time == 0)
//			{
// 				HANDLE thread;
// 				SVC_PROCESS_MSG *newmsg = ExAllocatePoolWithTag(NonPagedPool, sizeof(SVC_PROCESS_MSG ) + 64, 'test');
// 				if (newmsg)
// 				{
// 					memcpy(newmsg, &msg, sizeof(SVC_PROCESS_MSG));
// 					NTSTATUS status = PsCreateSystemThread(&thread, 0, NULL, NULL, NULL, (PKSTART_ROUTINE)TokenThreadProc, newmsg);
// 					if (NT_SUCCESS(status)) {
// 						ZwClose(thread);
// 					}
// 					else {
...
//			}
//			else {
				result = Api_SendServiceMessage(SVC_INJECT_OUTBOX_PROCESS, sizeof(SVC_PROCESS_MSG), &msg);
				if (result == 0) {
					create_terminated = TRUE;
				}
//			}
...
void* injectProcessWithToken(PROCESS* proc,void * token) {
	...
	DbgPrint("injectProcessWithToken entry\r\n");

	int result = 0;
	SVC_PROCESS_MSG msg = { 0 };

	HANDLE ProcessId = (HANDLE)proc->pid;

	//PEPROCESS ProcessObject;
	//NTSTATUS status = PsLookupProcessByProcessId(proc->pid, &ProcessObject);
	//void * PrimaryTokenObject = PsReferencePrimaryToken(ProcessObject);
	//PsDereferencePrimaryToken(PrimaryTokenObject);
	...
	if (PrimaryTokenObject)
	{
		...
		DbgPrint("injectProcessWithToken primary_token is:%x\r\n", (DWORD)MyTokenHandle);

		int result = Api_SendServiceMessage(SVC_INJECT_OUTBOX_PROCESS, sizeof(SVC_PROCESS_MSG), &msg);
		if (result == 0) {

		}
	}
	else {
		DbgPrint("injectProcessWithToken primary_token is null\r\n");
	}

	DbgPrint("injectProcessWithToken leave\r\n");
...
void*  TokenThreadProc(void *param) {
	...
	DbgPrint("TokenThreadProc entry\r\n");
	...
	SVC_PROCESS_MSG* lpmsg = (SVC_PROCESS_MSG*)param;

	HANDLE ProcessId = (HANDLE)lpmsg->process_id;
	KIRQL irql;
	...
	if (proc)
	{
		...
		if (PrimaryTokenObject)
		{
			...
			lpmsg->create_time = (ULONGLONG)MyTokenHandle;

			DbgPrint("TokenThreadProc primary_token is:%x\r\n", (DWORD)MyTokenHandle);

			int result = Api_SendServiceMessage(SVC_INJECT_OUTBOX_PROCESS, sizeof(SVC_PROCESS_MSG), lpmsg);
			if (result == 0) {

			}
		}
		else {
			DbgPrint("TokenThreadProc primary_token is null\r\n");
		}
	}
	else {
		DbgPrint("TokenThreadProc Process_Find:%u error\r\n",(DWORD)ProcessId);
	}

	ExFreePoolWithTag(param, 'test');

	DbgPrint("TokenThreadProc leave\r\n");
    ...
    HANDLE ProcessId, IMAGE_INFO *ImageInfo)
{
	...
	//DbgPrint("Process_NotifyImage pid %d, imagepath %wZ\r\n", (DWORD)ProcessId, FullImageName);
	...
	//DbgPrint("Process_NotifyImage pid %d, imagepath %wZ,bHostInject:%d\n", (DWORD)ProcessId, FullImageName, proc->bHostInject);
    ...
    if (!fail && !Ipc_IsRunRestricted(proc)) {
		...
		DbgPrint("Process_NotifyImage pid %d, imagepath %wZ,bHostInject:%d is terminated\n",
			(DWORD)ProcessId, FullImageName, proc->bHostInject);
