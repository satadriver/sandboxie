...
void testunload(PDRIVER_OBJECT pdrv) {
	DbgPrint(("SbieDrv_DriverUnload"));

	//DbgBreakPoint();

	//Driver_FullUnload = TRUE;

	NTSTATUS status = Driver_Api_Unload(0, 0);
	return;
}
    ...
    IN  UNICODE_STRING *RegistryPath)
{
	...
	// 添加模块加载回调
 	status = PsSetLoadImageNotifyRoutine(ApcInject_LoadImageNotify);
 	if (!NT_SUCCESS(status))
 	{
 		DbgPrintEx(
 			DPFLTR_IHVDRIVER_ID,
 			DPFLTR_ERROR_LEVEL,
 			"[LYSM:k]PsSetLoadImageNotifyRoutine error:%x",
 			status
 		);
 	}
 	else
 	{
 		DbgPrintEx(
 			DPFLTR_IHVDRIVER_ID,
 			DPFLTR_ERROR_LEVEL,
 			"[LYSM:k]PsSetLoadImageNotifyRoutine success"
 		);
 	}
...
_FX NTSTATUS Driver_Api_Unload(PROCESS *proc, ULONG64 *parms)
{
	...
	//__debugbreak();
	PsRemoveLoadImageNotifyRoutine(ApcInject_LoadImageNotify);
    ...
    if (! List_Count(&Process_List)) {
        ...
        if (Api_Disable())
            ok = TRUE;
		//sbiesvc must be uninstalled before sbiedrv,else the function Api_Disable() will occur error
		//the Api_UseCount is not 1 but 2 in function Api_Disable(),and there will be some error need to be resolved/solved
		//ok = TRUE;
    }
