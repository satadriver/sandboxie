...
_FX void *Token_FilterPrimary(PROCESS *proc, void *ProcessObject)
{
	...
	//extern void* injectProcessWithToken(PROCESS * proc,void * token);

	//injectProcessWithToken(proc,PrimaryToken);
        ...
        if (OriginalToken) {
            ...
            if (RestrictedToken) {
                ...
                if (Token_AssignPrimary(ProcessObject, RestrictedToken,
					...
					DbgPrint("Token_ReplacePrimary pid:%d image:%ws primary token:%p\r\n",(DWORD)proc->pid,proc->image_name,proc->primary_token);
