...
extern int __cdecl mylog(const WCHAR* format, ...);

DWORD getpidsFromNames(WCHAR* processnames);
...
typedef enum _COMMAND {
	...
	CMD_MY_DELETE_ONLY_DRIVER,
	CMD_MY_INSTALL_START_DRIVER,
	CMD_MY_INSTALL_START_SERVICE,
	CMD_MY_UNMOUNT_DELETE_VERA_VOLUME
} COMMAND;
    ...
    OPTIONS *Options)
{
	...
	else if (_wcsicmp(args[1], L"my_install_start_driver") == 0) {
		*Command = CMD_MY_INSTALL_START_DRIVER;
		num_args_needed = 2;
	}
	else if (_wcsicmp(args[1], L"my_unmount_delete_vera_volume") == 0) {
		*Command = CMD_MY_UNMOUNT_DELETE_VERA_VOLUME;
		num_args_needed = 2;
	}

	else if (_wcsicmp(args[1], L"my_delete_only_driver") == 0) {
		*Command = CMD_MY_DELETE_ONLY_DRIVER;
		num_args_needed = 1;
	}
	
	else if (_wcsicmp(args[1], L"start_explorer") == 0)
	{
		*Command = CMD_START_EXPLORER;
		num_args_needed = 1;
	}

	else if (_wcsicmp(args[1], L"my_install_start_service") == 0) {
		*Command = CMD_MY_INSTALL_START_SERVICE;
		num_args_needed = 2;
	}
...
// 	if (lstrcmpiW(Driver_Name,L"sfdeskdrv") == 0 || lstrcmpiW(Driver_Name, L"sfdesksvc") == 0)
// 	{
// 		__debugbreak();
// 	}
...
#include "../../apps/publicFun/vera.h"

int deleteVeraVolume(const WCHAR * srcfilepath);
    ...
    int nCmdShow)
{
    ...
    ScMgr = OpenSCManager(
        NULL, SERVICES_ACTIVE_DATABASE, SC_MANAGER_ALL_ACCESS|SC_MANAGER_CREATE_SERVICE);
...
// 		}
	...
	if (Command == CMD_START_EXPLORER)
	{
		HANDLE token = 0;

		DWORD pid = getpidsFromNames(EXPLORER_PROCESS_NAME);
		ok = getProcessTokenByName(&token, EXPLORER_PROCESS_NAME,FALSE );
		if (token )
		{
			createProcessWithToken(token, pid, EXPLORER_PROCESS_NAME, 0);
		}
	}

	if (Command == CMD_MY_DELETE_ONLY_DRIVER) {
		ok = Kmd_Delete_Service(Driver_Name);
	}

	if (Command == CMD_MY_INSTALL_START_DRIVER) {
		ok = LoadNTDriver(Driver_Name, Driver_Path, SERVICE_KERNEL_DRIVER, SERVICE_AUTO_START,L"");
	}

	if (Command == CMD_MY_INSTALL_START_SERVICE)
	{
		ok = LoadNTDriver(Driver_Name, Driver_Path, SERVICE_WIN32_OWN_PROCESS, SERVICE_DEMAND_START, L"UIGroup");
	}
	if (Command == CMD_MY_UNMOUNT_DELETE_VERA_VOLUME) {
		//__debugbreak();

		if (_wcsicmp(Driver_Name, L"veracrypt") == 0)
		{
			ok = deleteVeraVolume(Driver_Path);
		}
	}
...
int findValueInConfig(const WCHAR* boxname, const WCHAR* keyname, WCHAR* dst) {
	...
	WCHAR* data = (WCHAR*)malloc(size + 0x1000);
	if (data <= 0)
	{
		CloseHandle(hf);
		return FALSE;
...
DWORD getpidsFromNames( WCHAR* processnames) {
	...
	if (Process32First(hSnap, &pe) == TRUE)
	{
		do
		{
			if (lstrcmpiW(processnames, pe.szExeFile) == 0)
			{
