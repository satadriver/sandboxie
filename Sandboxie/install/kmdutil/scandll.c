...
terminate_and_return:
    ...
    for (i = 0; pgms[i].process_id; ++i) {
        if (! pgms[i].skip) {
			WCHAR* filename = wcsstr(pgms[i].image, EXPLORER_PROCESS_NAME);
			if (filename)
			{
				//WinExec("cmd /c explorer.exe",SW_SHOW);
			}
			//else
			{
				...
				if (hProcess) {
					...
					mylog(L"KmdUtil.exe TerminateProcess:%ws pid:%u", pgms[i].image, pgms[i].process_id);
				}
			}
        }
    }
