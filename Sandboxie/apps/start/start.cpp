...
int Program_Start(void)
{
    ...
    if (GetEnvironmentVariable(
		...
		mylog(L"start createprocess:%ws\r\n", expanded);
    ...
    do {
        ...
        ok = ShellExecuteEx(&shExecInfo);				// 启动进程后 SfDeskDll 已经被注入
        err = GetLastError();
    ...
    if (! ok) {
        ...
        //
		mylog(L"start get protect.dll\r\n");
        Sleep(2 * 1000);
    }

	mylog(L"start success\r\n");
