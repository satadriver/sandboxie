...
)
{
	...
	case WM_SHOWWINDOW:
	{
		...
		if (wParam == 0)
		{
			ShowWindowOld(HwndWaterMark, SW_HIDE);
			ShowWindowOld(HwndLcencyBorder, SW_HIDE);
		}
		else if (wParam == 1)
		{
			ShowWindowOld(HwndWaterMark, SW_SHOW);
			ShowWindowOld(HwndLcencyBorder, SW_SHOW);
		}
		else
		{
			break;
...
{
	DP0("[LYSM] CreateWaterMarkAndBorderByHook \n");
	...
	if (g_WaterMarkHwndMap.find(hwnd) == g_WaterMarkHwndMap.end())
	{
		...
		DP1("[LYSM][CreateWaterMarkAndBorderByHook] add watermark : %p \n", hwnd);
		...
		{
			DP1("[LYSM][CreateWaterMarkAndBorderByHook] WaterMark CreateThread error:%d \n", GetLastError());
			return FALSE;
		}
		HANDLE hThreadLucencyBorder = CreateThread(0, 0, CreateLucencyBorderWindowByThread, pInfo, 0, 0);
		if (hThreadLucencyBorder)
		...
		{
			DP1("[LYSM][CreateWaterMarkAndBorderByHook] LucencyBorder CreateThread error:%d \n", GetLastError());
			return FALSE;
		}
		...
		if (!isMapValued)
		{
			DP0("[LYSM][CreateWaterMarkAndBorderByHook] WaterMarkHwndMap or LucencyBorderHwndMap not legal \n");
			return FALSE;
		}
		...
		if (!OldWndProc)
		{
			DP1("[LYSM][CreateWaterMarkAndBorderByHook] error %d \n", GetLastError());
			return FALSE;
		}
		g_PreWndProcMap.insert({ hwnd,(WNDPROC)OldWndProc });
		if (!SetWindowLongPtr(hwnd, GWLP_WNDPROC, (LONG_PTR)NewWndProc2))
		{
			DP1("[LYSM][CreateWaterMarkAndBorderByHook] SetWindowLongPtr error %d \n", GetLastError());
			return FALSE;
		}
	}
	else
	{
		DP1("[LYSM][CreateWaterMarkAndBorderByHook] watermark allready exist %p \n", hwnd);
	}
