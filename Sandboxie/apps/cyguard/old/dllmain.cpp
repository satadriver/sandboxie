...
extern "C" __declspec(dllexport) int __stdcall hookfunctions() {
	...
	{
		int num = 0;
		int targetprocess = isTargetProcess(g_injectProcess, szapp, &num);
		if (targetprocess)
		{
// 			result = hook(L"gdi32.dll", L"GetDIBits", (LPBYTE)GetDIBitsNew, (PROC*)&GetDIBitsOld);
// 			result = hook(L"gdi32.dll", L"BitBlt", (LPBYTE)BitBltNew, (PROC*)&BitBltOld);
// 			result = hook(L"gdi32.dll", L"CreateDCW", (LPBYTE)CreateDCWNew, (FARPROC*)&CreateDCWOld);
// 			result = hook(L"gdi32.dll", L"CreateDCA", (LPBYTE)CreateDCANew, (FARPROC*)&CreateDCAOld);
// 			result = hook(L"User32.dll", L"GetWindowDC", (LPBYTE)GetWindowDCNew, (FARPROC*)&GetWindowDCOld);
// 			result = hook(L"User32.dll", L"GetDC", (LPBYTE)GetDCNew, (FARPROC*)&GetDCOld);
// 			result = hook(L"User32.dll", L"PrintWindow", (LPBYTE)PrintWindowNew, (FARPROC*)&PrintWindowOld);
		}
	}
	...
	if (bInSandBox)
	{
		if (lstrcmpiW(filename, EXPLORER_PROCESS_NAME_PLUSPLUS) == 0)
		{
